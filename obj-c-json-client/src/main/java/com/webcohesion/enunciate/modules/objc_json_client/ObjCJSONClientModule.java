 ...
 * @author Ryan Heaton
 */
public class ObjCJSONClientModule extends BasicGeneratingModule
  ...
  public void call(EnunciateContext context) {
    ...
    if (usesUnmappableElements()) {
      warn(
          "Web service API makes use of elements that cannot be handled by the Objective-C XML client. Objective-C XML client will not be generated.");
      return;
    }
    ...
    if (namingConflicts != null && !namingConflicts.isEmpty()) {
      ...
      error(
          "These naming conflicts are often between the field and it's associated property, in which case you need to use one or two of the following strategies to avoid the conflicts:");
      error("1. Explicitly exclude one or the other.");
      error("2. Put the annotations on the property instead of the field.");
      error(
          "3. Tell JAXB to use a different process for detecting accessors using the @XmlAccessorType annotation.");
    ...
    NameForTypeDefinitionMethod nameForTypeDefinition = new NameForTypeDefinitionMethod(
        getTypeDefinitionNamePattern(), slug, jaxbContext.getNamespacePrefixes(),
        packageIdentifiers);
    model.put("nameForTypeDefinition", nameForTypeDefinition);
    model.put("nameForEnumConstant", new NameForEnumConstantMethod(getEnumConstantNamePattern(),
        slug, jaxbContext.getNamespacePrefixes(), packageIdentifiers));
    ...
    Set<String> facetIncludes = new TreeSet<String>(
        this.enunciate.getConfiguration().getFacetIncludes());
    facetIncludes.addAll(getFacetIncludes());
    Set<String> facetExcludes = new TreeSet<String>(
        this.enunciate.getConfiguration().getFacetExcludes());
        ...
        for (TypeDefinition complexType : schemaInfo.getTypeDefinitions()) {
          for (Attribute attribute : complexType.getAttributes()) {
            if (attribute.isXmlList()) {
              info(
                  "%s: The Objective-C JSON client code won't serialize xml lists as an array, instead passing the list as a string that will need to be parsed. This may cause confusion to C consumers.",
                  positionOf(attribute));
            }

            if (attribute.isCollectionType() && attribute.isBinaryData()) {
              warn(
                  "%s: The Objective-C JSON client code doesn't support a collection of items that are binary data. You'll have to define separate accessors for each item or disable the C module.",
                  positionOf(attribute));
          ...
          if (complexType.getValue() != null) {
            if (complexType.getValue().isXmlList()) {
              info(
                  "%s: The Objective-C JSON client code won't serialize xml lists as an array, instead passing the list as a string that will need to be parsed. This may cause confusion to C consumers.",
                  positionOf(complexType.getValue()));
            }

            if (complexType.getValue().isCollectionType()
                && complexType.getValue().isBinaryData()) {
              warn(
                  "%s: The Objective-C JSON client code doesn't support a collection of items that are binary data.",
                  positionOf(complexType.getValue()));
              usesUnmappableElements = true;
            }
          }
          ...
          for (Element element : complexType.getElements()) {
            ...
            if (element.getAccessorType() instanceof MapType && !element.isAdapted()) {
              warn(
                  "%s: The Objective-C JSON client doesn't have a built-in way of serializing a Map. Use @XmlJavaTypeAdapter to supply your own adapter for the Map.",
                  positionOf(element));
              usesUnmappableElements = true;
            }
            ...
            if (element.isCollectionType()) {
              if (element.getChoices().size() > 1) {
                info(
                    "%s: The Objective-C JSON client code doesn't fully support multiple choices for a collection. It has to separate each choice into its own array. "
              ...
              for (Element choice : element.getChoices()) {
                if (choice.isNillable()) {
                  info(
                      "%s: The Objective-C JSON client code doesn't support nillable items in a collection (the nil items will be skipped). This may cause confusion to C consumers.",
                      positionOf(choice));
                }
              }
            }
   ...
   *          The root model.
   */
  public String processTemplate(URL templateURL, Object model)
  ...
  protected String readResource(String resource, Map<String, Object> model,
    ...
    URL res = ObjCJSONClientModule.class.getResource(resource);
    try {
  ...
  private TypeDefinition findRequestElement(Method exampleResource) {
    if (exampleResource.getRequestEntity() != null) {
      for (MediaTypeDescriptor mediaTypeDescriptor : exampleResource.getRequestEntity()
          .getMediaTypes()) {
  ...
  private TypeDefinition findResponseElement(Method exampleResource) {
    if (exampleResource.getResponseEntity() != null) {
      for (MediaTypeDescriptor mediaTypeDescriptor : exampleResource.getResponseEntity()
          .getMediaTypes()) {
        if (EnunciateJaxbContext.SYNTAX_LABEL.equals(mediaTypeDescriptor.getSyntax())) {
          DataTypeReference dataType = mediaTypeDescriptor.getDataType();
          if (dataType instanceof DataTypeReferenceImpl) {
  ...
  /**
   * Finds an example resource method, according to the following preference
   * order:
   *
   * <ol>
   * <li>The first method annotated with
   * {@link com.webcohesion.enunciate.metadata.DocumentationExample}.
   * <li>The first method with BOTH an output payload with a known XML element
   * and an input payload with a known XML element.
  ...
  protected URL getTemplateURL(String template) {
    return ObjCJSONClientModule.class.getResource(template);
  }
  ...
  public Map<String, String> getPackageIdentifiers() {
    ...
    for (HierarchicalConfiguration conversionElement : conversionElements) {
      conversions.put(conversionElement.getString("[@name]"),
          conversionElement.getString("[@identifier]"));
    }
    return conversions;
  }
   ...
   *
   * @return The format string creating a package identifier from a package
   *         name.
   */
   ...
   *
   * @return The pattern for converting a type definition to a unique C-style
   *         type name.
   */
   ...
   *
   * @return The pattern for converting an enum constant to a unique C-style
   *         type name.
   */
